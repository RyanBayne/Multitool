<?php         
/**
 * Multitool - Options Table Interface
 *
 ...
 * @version 1.0 
 */
class Multitool_Options {
    use Multitool_OptionsTrait;
}

trait Multitool_OptionsTrait {
    ...
    */
    private static $grouped_options = array(      
        'compact' => 'multitool_options',
        'private' => 'multitool_private_options',
    );
    ...
    */
    public static function get_option_names( $type = 'compact' ) {           
        switch ( $type ) {
            ...
            //Individual options here, will be prepended with "multitool".
            case 'non_compact' :
                return array(
        ...
        */
        ...
        switch ( $type ) {        
            case 'single':
                ...
                // Remember the real option names are prepend with "multitool".
                $single_options = array(  
                    // Multitool core options.                                  
    ...
    */
    ...
    /**
     * Returns the requested option.  Looks in multitool_options group 
     * or multitool_$name as appropriate.
     *
     * @version 1.2
    ...
    public static function get_option( $name, $default = false, $maybe_unserialize = true ) {
        ...
        if ( self::is_valid( $name, 'non_compact' ) ) {
            $option_value = get_option( "multitool_$name", $default );
        ...
        trigger_error( sprintf( 'Invalid Multitool option name: %s', $name ), E_USER_WARNING );
    ...
    */                               
     ...
     * Updates multitool_options or jetpack_$name as appropriate.
     *
     * @param string $name Option name
    ...
    public function update_option( $name, $value, $autoload = null ) {    
        if ( self::is_valid( $name, 'non_compact' ) ) {
            ...
            if ( version_compare( $GLOBALS['wp_version'], '4.2', '>=' ) ) {
                return update_option( "multitool_$name", $value, $autoload );
            }
            return update_option( "multitool_$name", $value );
        }
    ...
    public function update_options( $array ) {
        ...
        foreach ( array_diff( $names, self::get_option_names(), self::get_option_names( 'non_compact' ), self::get_option_names( 'private' ) ) as $unknown_name ) {
            trigger_error( sprintf( 'Invalid Multitool option name: %s', $unknown_name ), E_USER_WARNING );
     ...
     * Updates multitool_options and/or deletes multitool_$name as appropriate.
     *
    ...
    public function delete_option( $names ) {       
        ...
        if ( ! self::is_valid( $names ) ) {
            trigger_error( sprintf( 'Invalid Multitool option names: %s', print_r( $names, 1 ) ), E_USER_WARNING );

            return false;
        }

        foreach ( array_intersect( $names, self::get_option_names( 'non_compact' ) ) as $name ) {
            if ( ! delete_option( "multitool_$name" ) ) {
                $result = false;
            }
        }
